...
# 询问是否部署Worker
read -p "是否部署Worker? (y/n): " deploy_worker
if [[ $deploy_worker == "y" || $deploy_worker == "Y" ]]; then
    echo -e "${BLUE}[信息]${NC} 开始部署Worker..."
    bash "$SCRIPT_DIR/deploy_worker.sh"
else
    echo -e "${BLUE}[信息]${NC} 跳过Worker部署"
    echo -e "${YELLOW}[提示]${NC} 你可以稍后通过运行 ./deploy/deploy_worker.sh 来部署Worker"
fi

echo -e "${GREEN}[完成]${NC} 游戏文件上传完成"
